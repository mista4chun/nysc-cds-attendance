...
export default function CLOLayout({ children }: { children: React.ReactNode }) {
  ...
  const initials = profile?.full_name
    ?.split(' ').slice(0, 2).map(n => n[0]).join('') ?? 'CL'
      ...
      `}>
        ...
        <div className="h-14 flex items-center justify-between px-4 border-b border-gray-200">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-semibold text-gray-900 text-sm">NYSC Attendance</span>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
        ...
        <div className="px-4 py-3 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">
              <span className="text-green-800 text-sm font-semibold">{initials}</span>
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium text-gray-900 truncate">{profile?.full_name ?? '—'}</p>
              <p className="text-xs text-gray-500">CLO · {profile?.state_code ?? '—'}</p>
            </div>
          </div>
        </div>
