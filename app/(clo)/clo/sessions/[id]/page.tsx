...
export default async function SessionDetailPage({
  ...
  return (
    <div className="space-y-5 pb-20 lg:pb-6">
      <div>
        <Link href="/clo/sessions" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 mb-3">
          <ArrowLeft size={15} /> Back to sessions
        </Link>
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">{session.title}</h1>
            <p className="text-sm text-gray-500">{session.cds_groups?.name} · {session.location_name}</p>
          </div>
      ...
      <div className="grid gap-3 grid-cols-3">
        <div className="bg-white rounded-xl border border-gray-200 p-3 text-center">
          <p className="text-2xl font-bold text-green-700">{records?.length ?? 0}</p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 text-center">
          <p className="text-2xl font-bold text-gray-700">
            {memberCount ? Math.round(((records?.length ?? 0) / memberCount) * 100) : 0}%
          </p>
      ...
      <AttendanceList records={records ?? []} groupId={session.cds_group_id} sessionId={id} />
    </div>
  )
}